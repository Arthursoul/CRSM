...
from model.models.vqa_classifier import Classifier
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='COCO', choices=['COCO', 'VG_QA'])    
    parser.add_argument('--backbone_class', type=str, default='SwinT', choices=['Res12','SwinT','VitS'])
        ...
        if epoch > 100 or (epoch-1) % 5 == 0:
            ...
            with torch.no_grad():
                ...
                # for i, batch in tqdm(enumerate(val_loader, 1)):
                    ...
                    val_gen.set_description('验证阶段:epo {} vl_dist={:.4f} vl_sim={:.4f}  acc_dist={:.4f} acc_sim={:.4f}'.format(epoch, vl_dist.item(),vl_sim.item(),va_dist.item(), va_sim.item()))
